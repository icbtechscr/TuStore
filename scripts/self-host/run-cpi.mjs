...
export function checkSyncTarget(env) {
  ...
  if (!env.TUSTORE_SYNC_TARGET_ORIGIN || actual.origin !== new URL(env.TUSTORE_SYNC_TARGET_ORIGIN).origin ||
      actual.hostname.endsWith(".supabase.co")) {
    throw new Error("El worker de TuStore solo puede escribir en su Supabase propio autorizado");
  }
...
function main() {
  ...
  const days = Number(process.env.TUSTORE_SYNC_LOOKBACK_DAYS || "3");
  ...
  for (const script of jobs) {
    ...
    console.log(`[TuStore] Iniciando ${script} (${from}..${to})`);
    ...
    console.log(`[TuStore] ${script}: ${result.status === 0 ? "OK" : "ERROR"}`);
  }
  if (failures.length) throw new Error(`Fallaron: ${failures.join(", ")}`);
  console.log("[TuStore] Sincronización completa");
}
